...
Локальный дашборд (`npm run dashboard`, Node 18+, без `npm install`) над тем же markdown-vault — те же файлы открываются и в Obsidian. Дашборд имеет **две точки записи**: захват в инбокс (`POST /api/inbox`) и апрув задач review→done (`POST /api/approve`); остальное — чтение. Подробнее — [dashboard/README.md](dashboard/README.md).
...
2. Откройте папку в своём агенте и дайте ему инструкцию из раздела выше (Claude Code подхватывает правила через `CLAUDE.md`, который импортирует `@AGENTS.md`).
