...
# Огород (`ogorodush`): ИИ-команда и каноничная документация (черновик)

> Имя фреймворка: **Огород** (кириллица — для людей) / **`ogorodush`** (латиница —
...
> из APC). На 2026-06-13 `npm ogorodush` и `github.com/ogorodush` свободны.
...
бизнес-риск (бан, недоступность, цена). Огород снижает его двумя механизмами:
...
| Адоптер фреймворка | методология Огорода | выбрать нас |
...
Зависимость от одной топ-модели — точка отказа. Огород назначает задаче **tier
...
- шаблон (ogorodush) / внутренний инструмент (dashboard) → мерж в основную ветку.
...
Частый кейс: проект уже есть, человек переходит на Огород, документации нет или
...
Живёт **отдельным документом** `docs/styleguide.md` (канон Огорода, переносится в
...
агентом. Огород — методология, где дока ведётся сама, соразмерно важности.
