<!-- Audience: агент или контрибьютор Огорода, ведущий задачу от постановки до закрытия
...
В Огороде роли и проверки **включаются автоматически по важности задачи** — это
...
> **Адаптер-специфика.** Конкретный способ *фиксировать* стоимость и *механически блокировать* прыжок — дело адаптера (напр., Claude-адаптер llmush делает это через `scripts/task-checkpoint.py` + pre-commit-хук; этот скрипт — в llmush-vault, **не в шаблоне Огорода**). Канон Огорода — только маркер `doing`: он существует, он ставится до старта, он виден на борде.
...
### Обновление лога сессии при апруве

После того как человек переводит задачу в `done` (апрув через апрув-поверхность), запись в логе сессии карточки проекта должна отражать это:

- Было: `в review, ждёт апрува`
- Стало: `закрыто (#N)`

Если апрув-поверхность не пишет в лог сама — это делает агент в следующем касании проекта (ближайший `sync` или любая работа по проекту). Не оставляй запись в «подвешенном» состоянии дольше одного касания.

### created_at для legacy-задач

При переводе задачи без поля `created_at` (старая задача, может содержать `est_days` вместо `sp`) в статус `doing` или `review` — проставь `created_at`. Источники по убыванию приоритета: поле `created` в задаче → дата первого git-коммита по файлу → сегодняшняя дата. Без `created_at` валидатор (`scripts/check-task-close.py`) заблокирует коммит.
