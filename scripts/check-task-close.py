#!/usr/bin/env python3
"""Гейт закрытия задач (Огород): механически блокирует некорректные коммиты задач.

Четыре гейта:
1. DONE-апрув: агент не может закоммитить переход в `done` — только через апрув-
   поверхность адоптера (ставит OGOROD_APPROVE=1).
...
- cost_by_model / тиринг-чекпойнт: способ фиксировать стоимость задачи зависит
  от агента; канон Огорода только требует маркер `doing`, а не конкретного механизма.
...
- LLMUSH_APPROVE: в шаблоне флаг называется OGOROD_APPROVE (адаптер переименовывает).
"""
...
def duplicate_ids(staged: list[str]) -> list[tuple[str, list[str]]]:
    ...
    glob `projects/*/tasks/*.md` — стандартная структура Огорода; адаптер с
...
def main() -> int:
    # OGOROD_APPROVE=1 выставляет ТОЛЬКО апрув-поверхность (дашборд/CI), не агент.
    ...
    approved = bool(os.environ.get("OGOROD_APPROVE"))
    ...
    for f in staged:
        ...
        # OGOROD_APPROVE=1 разблокирует только этот гейт.
    ...
    if no_created:
        sys.stderr.write(
            "\n✋ Дата создания обязательна (гейт Огорода).\n\n"
    ...
    if dups:
        sys.stderr.write(
            "\n✋ Дубликат id задачи (гейт Огорода).\n\n"
    ...
    if skipped_doing:
        sys.stderr.write(
            "\n✋ Вход в работу: пропущен `doing` (гейт Огорода).\n\n"
            ...
            + "\nКанон Огорода: вход в работу = перевод задачи в `doing` ДО начала\n"
    ...
    if offenders:
        sys.stderr.write(
            "\n✋ Закрытие задачи заблокировано (гейт Огорода).\n\n"
            ...
            "OGOROD_APPROVE=1. Потолок агента — `review`.\n\n"
